...
## Summarize and plot developmental timepoints
...
dev = counts %>%
	...
	mutate(
		Isoform = case_when(
			Exon=="chr7_59243350_59243469" ~ "Long",
			Exon=="chr7_59247173_59247217" ~ "Short",
			T ~ "else"
			)
	) %>%
	select(-RNA,-Exon) %>%
	pivot_wider(names_from=Isoform,values_from=Expression) %>%
	mutate(ShortFraction = Short/(Short+Long)) %>%
	mutate(LongFraction = Long/(Short+Long)) %>%
	select(-Long,-Short) %>%
